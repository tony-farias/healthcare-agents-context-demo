...
def retrieve_context(transformed_patient: str, governed: bool) -> list[dict[str, Any]]:
    if not governed:
        selected = [dict(policy) for policy in POLICIES]
...
def generate_answer(config: ReliabilityConfig, policies: list[dict[str, Any]], tool_result: dict[str, Any]) -> str:
...
def write_memory(config: ReliabilityConfig, answer: str) -> str:
    ...
    _annotate(**{"workshop.memory_policy": "allowlisted_summary" if config.safe_memory else "raw_answer"})
    return value
...
def run_reliability_agent(config: ReliabilityConfig) -> dict[str, Any]:
    ...
    policies = retrieve_context(transformed, config.governed_retrieval)
    tool = select_tool(config, authorization)
    tool_result = validate_tool_result(tool, policies)
    model_context = f"Request: {QUERY}\nPatient context: {transformed}\nPolicies: {policies}\nTool: {tool_result}"
    answer = generate_answer(config, policies, tool_result)
